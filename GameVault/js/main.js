...
(function(){
  ...
  // Theme toggle: stores 'dark' or 'light'
  ...
  document.addEventListener('click', e => {
    if(e.target.closest('#theme-toggle') || e.target.closest('#theme-toggle-2') || e.target.closest('#theme-toggle-3')){
      const cur = localStorage.getItem(THEME_KEY) || 'dark';
      applyTheme(cur === 'dark' ? 'light' : 'dark');
    }
  });

  // Simple lazy loader for images (IntersectionObserver)
  ...
  window.observeLazy = function(root = document){
    root.querySelectorAll('img[data-src]').forEach(img => lazyObserver.observe(img));
  };

  // Favorites helper
  const F_KEY = 'gv_favs';
  window.getFavs = () => JSON.parse(localStorage.getItem(F_KEY) || '[]');
  window.isFav = id => getFavs().includes(id);
  window.toggleFav = id => {
    const arr = getFavs();
    const idx = arr.indexOf(id);
    if(idx === -1) arr.push(id); else arr.splice(idx,1);
    localStorage.setItem(F_KEY, JSON.stringify(arr));
    return arr;
  };
